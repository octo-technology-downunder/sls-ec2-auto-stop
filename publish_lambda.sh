...
if [[ ${lambda_exists} ]]; then
    ...
    lambda_version=$(increment_version $(aws serverlessrepo get-application --application-id ${lambda_id} --query "Version.SemanticVersion" --output text --region ap-southeast-2) 1)
